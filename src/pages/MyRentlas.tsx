...
const MyRentals = () => {
  ...
  const applyCoupon = () => {
    if (!couponCode) {
      ...
      const discountPercentage = parseInt(couponCode.replace("%", ""), 10); // Assuming coupon code is a percentage like "10%"

      const updatedRentals = unpaidRentals.map(
        (rental: { totalCost: number }) => {
          const discountedPrice =
            rental.totalCost - (rental.totalCost * discountPercentage) / 100;
  ...
  const paidRentals = data?.filter(
    (rental: { isPaid: boolean }) => rental.isPaid
  );
  const unpaidRentals = data?.filter(
    (rental: { isPaid: boolean }) => !rental.isPaid
  );
  ...
  return (
    <div className="p-6">
      ...
      >
        <TabsList>
          <TabsTrigger value="unpaid" aria-selected={activeTab === "unpaid"}>Unpaid</TabsTrigger>
          <TabsTrigger value="paid" aria-selected={activeTab === "paid"}>Paid</TabsTrigger>
        </TabsList>
              ...
              (rental: {
                ...
                <div key={rental.id} className="p-4 border-b">
                  ...
                  <p>
                    Total Cost: $
                    {discountedRentals
                      .find((d) => d.id === rental._id)
                      ?.discountedPrice.toFixed(2) ??
                      rental.totalCost.toFixed(2)}
                  </p>
                  {/* Show coupon application option */}

                  <Button className="mr-2" onClick={applyCoupon}>
                    Apply Coupon
                  </Button>
